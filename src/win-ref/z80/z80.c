...
// opcode (M1) fetch: routed through fetch_opcode when present so encrypted
// CPUs can return decrypted opcodes distinct from data reads.
static inline uint8_t fetchb(z80* const z) {
  if (z->fetch_opcode)
    return z->fetch_opcode(z->userdata, z->pc++);
  return rb(z, z->pc++);
}
...
void exec_opcode(z80* const z, uint8_t opcode) {
  ...
  case 0xCB: exec_opcode_cb(z, fetchb(z)); break;
  case 0xED: exec_opcode_ed(z, fetchb(z)); break;
  case 0xDD: exec_opcode_ddfd(z, fetchb(z), &z->ix); break;
  case 0xFD: exec_opcode_ddfd(z, fetchb(z), &z->iy); break;
