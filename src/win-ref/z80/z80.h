...
struct z80 {
  ...
  // Optional separate opcode (M1) fetch. Needed by encrypted CPUs such as
  // the NEC MC-8123, where opcode fetches and data reads see different bytes.
  // If NULL, opcode fetches fall back to read_byte.
  uint8_t (*fetch_opcode)(void*, uint16_t);
  void* userdata;
